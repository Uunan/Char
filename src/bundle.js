...
`;
  ...
  function updateActiveStates(ed) {
    const toggle = (id, isActive2) => {
      const el = document.getElementById(id);
      if (el) el.classList.toggle("active", isActive2);
    };
  ...
  function scrollToMatch(idx) {
    ...
    const { from: from2, to } = findMatches[idx];
    editor.chain().focus().setTextSelection({ from: from2, to }).run();
  ...
  function replaceCurrent() {
    ...
    if (!term) return;
    const { from: from2, to } = findMatches[findCurrentIdx];
    editor.chain().focus().deleteRange({ from: from2, to }).insertContentAt(from2, replacement).run();
    findInDocument(term);
  }
  function replaceAll() {
    if (isReadOnly) return;
    ...
    reversed.forEach(({ from: from2, to }) => {
      chain.deleteRange({ from: from2, to }).insertContentAt(from2, replacement);
    });
    chain.run();
    findInDocument(term);
  }
  ...
  function updateUIForUser() {
    if (currentUser) {
      ...
      document.getElementById("btn-read-mode").textContent = "\u{1F4D6} Okuma Modundan \xC7\u0131k";
    } else {
      document.getElementById("user-section").style.display = "none";
      document.getElementById("readonly-badge").style.display = "inline-block";
      document.getElementById("btn-read-mode").textContent = "\u{1F4D6} Sadece Okuma";
    }
  }
  document.getElementById("menu-new").addEventListener("click", async () => {
    if (isReadOnly) return triggerLoginFlow();
    if (isDirty) {
      const ok = confirm("Kaydedilmemi\u015F de\u011Fi\u015Fiklikler var. Yeni belge a\xE7\u0131ls\u0131n m\u0131?");
      if (!ok) return;
    }
